...
const URL = process.env.REACT_APP_BACKEND_URL + "/api/changePassword";
  ...
  const handleChangePassword = async (ev) => {
    ...
    if (newpassword !== confirmpassword){
      toast.error("Passwords do not match !");
        ...
        const formData = { email: email, password: newpassword, oldpassword : oldpassword };
  ...
  return (
    <div className="w-full flex justify-center my-4">
      <div className="w-full max-w-lg p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <form className="w-full flex max-w-md flex-col gap-4"onSubmit={handleChangePassword}>
          <div>
            <div className="mb-2 block">
              <label htmlFor="oldpassword" className="text-sm font-medium required">
                Old Password
              </label>
            </div>
            <input id="oldpassword" name="oldpassword" type="password" placeholder="Old Password" required
